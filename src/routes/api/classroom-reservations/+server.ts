...
export async function POST(event) {
    ...
    `;
    let params = [classroom_id, reservation_date, start_time, end_time];
    let conflictResult = await query(conflictQuery, params);

    // 학기 내 스케줄 충돌 확인
    const semesterCode = await getSemesterCode(reservation_date);
    if (semesterCode) {
        const scheduleQuery = `
            ...
            AND s.semester = $5
        `;
        const scheduleParams = [classroom_id, reservation_date, start_time, end_time, semesterCode];
        const scheduleResult = await query(scheduleQuery, scheduleParams);
        conflictResult = [...conflictResult, ...scheduleResult];
    }
