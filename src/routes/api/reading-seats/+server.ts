...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        // // user_id 형식 검증 (예: A로 시작, 6~20자)
        // if (!/^A[0-9]{6,19}$/.test(user_id)) {
        //   return new Response('잘못된 사용자 ID 형식입니다.', { status: 400 });
        // }
        // user_id 형식 검증 (7자리 또는 10자리 문자와 숫자 조합)
        if (!/^([a-zA-Z0-9]{7}|[a-zA-Z0-9]{10})$/.test(user_id)) {
            return new Response('잘못된 사용자 ID 형식입니다. 7자리 또는 10자리 문자와 숫자 조합이어야 합니다.', { status: 400 });
        }
        // 좌석 번호 유효성 체크
        if (seat < 1 || seat > 60) {
            return new Response('잘못된 좌석 번호입니다.', { status: 400 });
        }
...
export const DELETE: RequestHandler = async ({ request }) => {
    try {
        ...
        if (check.length === 0) {
            return new Response('좌석을 찾을 수 없거나 권한이 없습니다.', { status: 403 });
        }
